...
class Solution {
	...
	public static int maxWin = 0, n, diceIndex = 0, maxDiceCount;
	public int[] solution(int[][] dice) {
		n = dice.length;
		goat = new int[n / 2];
		playerA = new int[n / 2];
		maxDiceCount = (int)Math.pow(6, n / 2);
	...
	public static void solve(int[][] dice, int[] playerA) {
		int[] playerADiceResult = new int[maxDiceCount];
		int[] playerBDiceResult = new int[maxDiceCount];
		diceIndex = 0;
		...
		for (int i = 0; i < maxDiceCount; i++) {
			while (playerBIndex < maxDiceCount && 
